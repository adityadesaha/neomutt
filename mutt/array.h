 ...
 * @param head Pointer to a struct defined using ARRAY_HEAD
 * @retval elem Pointer to the first element
 ...
 * @param head Pointer to a struct defined using ARRAY_HEAD
 * @retval elem Pointer to the last element
 * @retval NULL Array is empty
 */
...
#define ARRAY_REMOVE(head, elem) \
  (memmove((elem), \
           ...
           (ARRAY_SIZE((head)) - ARRAY_IDX((head), (elem)))), \
   ARRAY_SHRINK((head), 1))

/**
 * ARRAY_SORT - sort an array
 * @param head Pointer to a struct defined using ARRAY_HEAD
 * @param fn   Sort function, see qsort(3) for its signature
 */
